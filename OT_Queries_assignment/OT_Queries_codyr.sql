...
---------------------------------------------------------------------------------------
select  region_id, count(region_id) as 'Country Count'  from countries
group by region_id
...
---------------------------------------------------------------------------------------
select countries.region_id, count(countries.region_id) as 'Country Count', regions.region_name as 'region with most locations' from countries
join regions
...
---------------------------------------------------------------------------------------
select state, city, postal_code from locations
...
where state is null;  
## if this question is just adding to the above query, 'and state is null' can be added to the where line
---------------------------------------------------------------------------------------
/* 	**
...
where list_price > 100 and list_price < 500
...
---------------------------------------------------------------------------------------
select products.product_name, products.list_price, category_name from products
join product_categories as pc
on pc.category_id = products.category_id
order by product_id;  
---------------------------------------------------------------------------------------
/* 	****
...
where warehouse_name = 'Toronto' and quantity > 100 and category_name = 'CPU'
order by list_price;
---------------------------------------------------------------------------------------
/* 	**
